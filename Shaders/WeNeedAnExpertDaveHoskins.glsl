...
//  h += 0.5*texture(iChannel1, 2.0*p).x;
  ...
  vec2 fp = 20.0*p;
  float fft = texture(iChannel0, vec2(0.1+0.8*fract(fp.x+0.25*fp.y+0.25*sign(s)), 0.25)).x;
  fft -= 0.3;
  fft = max(fft, 0.0);
  fft *= fft;
//  fft *= fft;
...
vec3 effect(vec2 p, vec2 pp) {
  ...
  float fft = texture(iChannel0, vec2(0.1+0.8*fract(off+0.25*sin(fov)), 0.25)).x;
