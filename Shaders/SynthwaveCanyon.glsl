...
vec4 plane(vec3 ro, vec3 rd, vec3 pp, vec3 off, float aa, float n) {
  ...
  float snd = freq(s);
  vec3 hn;
  vec2 p = (pp-off*2.0*vec3(1.0, 1.0, 0.0)).xy*(.5+snd*.25);
  ...
  vec3 hsv = vec3(fract(0.7+0.125*sin(0.6*pp.z*snd)), 0.5, smoothstep(aa, -aa, abs(d)-aa));
...
vec3 skyColor(vec3 ro, vec3 rd) {
  ...
  vec3 glow = mix(vec3(snd, 0.7*snd, 0.6*snd).zyx, glareCol, ss);
  ...
  vec3 corona = 0.65*glow*exp(-2.5*d)*ss*snd*2.;
  col += corona;
