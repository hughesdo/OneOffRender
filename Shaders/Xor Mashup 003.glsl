...
void main() {
    ...
    // Compute treble (high frequencies)
    float treble = 0.0;
    for (float i = TREBLE_FREQ_START_BIN; i <= TREBLE_FREQ_END_BIN; i++) {
        float sample_pos = i / FFT_SIZE;
        treble += texture(iChannel0, vec2(sample_pos, WAV_BLEED)).r;
    }
    float treble_num_bins = TREBLE_FREQ_END_BIN - TREBLE_FREQ_START_BIN + 1.0;
    treble = treble / treble_num_bins;
    
    // Compute bass (low frequencies: bins 1 to 5)
    float bass = 0.0;
    for (float i = BASS_FREQ_START_BIN; i <= BASS_FREQ_END_BIN; i++) {
        float sample_pos = i / FFT_SIZE;
        bass += texture(iChannel0, vec2(sample_pos, WAV_BLEED)).r;
    }
    float bass_num_bins = BASS_FREQ_END_BIN - BASS_FREQ_START_BIN + 1.0;
    bass = bass / bass_num_bins;
    
    // Compute overall intensity (all bins: 0 to 511)
    float intensity = 0.0;
    for (float i = INTENSITY_FREQ_START_BIN; i <= INTENSITY_FREQ_END_BIN; i++) {
        float sample_pos = i / FFT_SIZE;
        intensity += texture(iChannel0, vec2(sample_pos, WAV_BLEED)).r;
    }
    float intensity_num_bins = INTENSITY_FREQ_END_BIN - INTENSITY_FREQ_START_BIN + 1.0;
    intensity = intensity / intensity_num_bins;
