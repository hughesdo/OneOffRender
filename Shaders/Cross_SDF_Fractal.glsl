...
// Original author: bradjamesgrant
// Modified by: ArthurTent for ShaderAmp project
// URL: https://www.shadertoy.com/view/tssfz7
...
vec3 palette(float d, float intensity){
    ...
    return mix(vec3(sin(iTime)+intensity*10.3*cos(iTime),.51+intensity*0.5,intensity+2.2*sin(iTime)),vec3(.8,.2,.5),1.-d)*3.5;
}
float rnd(vec2 p){
...
vec3 castRay (vec3 ro, vec3 rd){
    float intensity = texture(iChannel0, vec2(0.25, 0.0)).r;
	float t=0.;
    vec3 col = vec3(0.);
    int i = 0;
    for(i = 0; i<50; i++){
