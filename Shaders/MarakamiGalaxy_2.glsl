...
// Original author: PixelPhil
// Modified by: ArthurTent for ShaderAmp project
// URL: https://www.shadertoy.com/view/3tyGRz
// License: Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported License
...
vec3 sphereColor(vec3 worldPos, float nDotV, float dist, float worldAngle)
{
    ...
    // Remap colors
    float colId = 3. * floor(mod(iTime/8.,4.));
    col = mat3(getCol(colId),getCol(colId+1.),getCol(colId+2.)) * col;

    col = mix(col, flPattern1.rgb*ffts.x, flPattern1.a*fft.x);
    col = mix(col, flPattern2.rgb*ffts.y, flPattern2.a*fft.w);
    col = mix(col, flPattern3.rgb*ffts.z, flPattern3.a*fft.z);
...
vec4 RenderPlanet(vec3 pos, float d, vec3 rayDir, float worldAngle, float coverage)
{
    ...
    //flowers.x *=fft.x; flowers.y *=fft.y; flowers.z *=fft.z;
...
vec3 render(vec3 camPos, vec3 rayDir, vec2 uv)
{
    ...
    //col = col * (1.0 - planetCol.a*ffts.w*0.5) + planetCol.rgb;
    //col = col * (1.0 - planetCol.a*fft.w) + planetCol.rgb;
