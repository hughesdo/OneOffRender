...
vec4 fft, ffts; //compressed frequency amplitudes
 ...
 // nice hint for loading tAudio --> https://threejs.org/examples/webaudio_visualizer
...
void camera(vec2 fragCoord, out vec3 ro, out vec3 rd, out mat3 t)
{
    ...
    //float ff = min(1.0, step(0.001, (iResolution.xy * 0.5).x) + step(0.001, (iResolution.xy * 0.5).y));
    float ff = min(1.0, step(0.001, (iResolution.xy * 0.5).x) + step(0.001, (iResolution.xy * 0.5).y))+sin(iTime/20.);
    vec2 m = PI*ff + vec2((((iResolution.xy * 0.5).xy + 0.1) / iResolution.xy) * (PI*2.0));
    //m.y = -m.y;
...
vec3 makeSym(vec2 p, float id){ //glyph definitions (lots of redundant code but individually tweakable), reference: https://seechangehappen.co.uk/gender-identity-pride-flags-glyphs-symbols-and-icons/
    ...
    mat3 t = mat3(1.0);
	camera(uv, ro, rd, t);
	vec3 vid = texture(iChannel0, (fragCoord / iResolution)).rgb;

	// Symbol
	float symId = hash21(vec2(ceil(iTime/8.))); //randomize symbol choice
	symId *= (fract(iTime/8.)<.75)? -1. : 1.; //define ratio between animated and static symbols
...
void main() {
	...
	float vBar,
	      ...
	      //spect = 6.*clamp(texelFetch(iChannel0,ivec2(int(511.*abs(fragCoord.y/iResolution.y-1.)),0),0).x-2.*abs(fragCoord.x/iResolution.x-.5),0.,1.); //audio spectrum pattern 
	      //spect = 6.*clamp(texelFetch(iChannel0,ivec2(int(511.*abs(fragCoord.y/iResolution.y-1.)),0),0).x-2.*abs(fragCoord.x/iResolution.x-.5),0.,1.); //audio spectrum pattern 
	      //spect = 6.*clamp(texelFetch(iChannel0,ivec2(int(511.*abs(uv)),0),0).x-2.*abs(uv-.5),0.,1.).y; //audio spectrum pattern 
	      //spect = 120.*clamp(texelFetch(iChannel0,ivec2(int(511.*abs(uv)),0),0).x-2.*abs(uv-.5),0.,1.).y; //audio spectrum pattern 
	      spect = 120.*clamp(texelFetch(iChannel0,ivec2(int(511.*abs(uv)),0),0).x-2.*abs(uv-.5),0.,1.).y; //audio spectrum pattern 
	uv = abs(10.*sin(iTime/64.)*uv); //zoom motion   
	for (float n=.0;n<3.;n++){ 
	...
	col = pow(col, vec3(.4545)); //gamma correction
				     //fragColor = vec4(col*vid.rgb,1.0);
				     //fragColor = vec4(col*(0.7+(sin(iTime))*vid.rgb),1.0);
				     //fragColor = vec4(col*(2.35*vid.rgb),1.0);
				     //fragColor = vec4(col*(2.*vid.rgb),1.0);
