...
uniform sampler2D iChannel1;  // Sky texture (for reflections)
uniform sampler2D iChannel2;  // Sky texture (for grain)
...
void main()
{
    ...
    float sphereColor	= 0.15 + 3.0 * pow( texture( iChannel0, vec2( 0.75, 0.25 ) ).x, 3.0 );

    if ( t1 > t0 )
    {
        t 				= t1;
    	sphereRadius 	= sphere1Radius;
        sphereColor		= 0.8 * pow( texture( iChannel0, vec2( 0.5, 0.25 ) ).x, 3.0 );
    ...
    if ( t2 > t1 )
    {
        ...
        sphereColor		= 0.1 * pow( texture( iChannel0, vec2( 0.0, 0.25 ) ).x, 3.0 );
    }
    ...
    if ( t > 0.0 )
    {
        ...
        color = texture( iChannel1, refl.xy ).xyz;
        color = color * color;
    ...
    //vec2 grainUV = ( fragCoord.xy / ( 2.0 * iResolution.xy ) + fract( vec2( iTime * 0.1, iTime * 0.3 ) ) );
    vec2 grainUV = ( fragCoord / ( 2.0 * iResolution.xy ) + fract( vec2( iTime * 0.1, iTime * 0.3 ) ) );
    float grain = texture( iChannel2, grainUV ).x;
