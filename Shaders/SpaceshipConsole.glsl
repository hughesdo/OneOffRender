...
void main(){
    ...
    vec2 uvNorm = fragCoord / iResolution;
    col -= clamp(-1.,.0,texelFetch( iChannel0, ivec2(512.*2.*abs(uvNorm.x-.5)*winDistort,0),0).x
        - 1.5*abs(uvNorm.y-.5)*winDistort) * (.8+.2*getCol(colId)); //sound texture "shield"
