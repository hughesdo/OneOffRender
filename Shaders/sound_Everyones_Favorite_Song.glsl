...
void main() {
    ...
    vec2 uv = -1.0 + 1.4 *(fragCoord / iResolution) + vec2(0.3, .4) ;
    float sndX = texture(iChannel0,vec2(uv.x+0.6)).x;
    float sndY = texture(iChannel0,vec2(uv.y+.3)).x;
    float vol = getVol(8.);
    ...
    || iResolution.y < 400. && t < 64.*TICK_DURATION){// Trying not to ruin the surprise in the preview.
        ...
        fragColor = vec4(saturate2(d / fwidth(d))) * .7;
        return;
    }
...
#define MIX(color,amount) c = mix(color, c, saturate((amount) * iResolution.y + .5))
    ...
    p.y = max(p.y+.1,0.);
    p /= .7;
    //MIX(WINDOW*vec3(.5-sin(10.*snd*iTime), snd*0.3, snd*cos(iTime)), max(max(
