...
// Created by fizzer
// Inspired by "The Popular Demo" by Farbrausch
...
vec3 squaresColours(vec2 p)
{
	...
	c+=vec3(0.1,0.5+0.5*cos(p*10.0*ffts.z))*vec3(0.05,0.1,0.4).bgr*0.7;

	//c +=fft.x*vec3(0.1,0.1,0.1);
	//c +=fft.x*vec3(1.-ffts.w,1.-ffts.w,1.-ffts.w);
	c +=fft.z*vec3(1.-ffts.w,1.-ffts.w,1.-ffts.w);
	return c;
}
...
vec3 scene(vec2 p)
{
	mat3 cam = rotateXMat(cos(time * 0.2) * 0.1) * rotateYMat(time * 0.5*fft.w); // straight forward
	//mat3 cam = rotateXMat(cos(time * 0.2) * 0.1*ffts.w) * rotateYMat(time * 0.5); // straight forward
	//mat3 cam = rotateXMat(cos(time * 0.2) * 0.1*ffts.w) * rotateYMat(time * 0.5*fft.w); // straight forward
	//mat3 cam = rotateXMat(cos(time * 0.2) * 1.1*ffts.w*fft.w) * rotateYMat(time * 0.5*fft.w);
