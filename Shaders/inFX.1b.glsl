...
// Original author: patu
// Modified by: ArthurTent for ShaderAmp project
// URL: https://www.shadertoy.com/view/ldd3Dr
...
vec4 bg ( in vec3 ePos, in vec3 eDir ) {
    ...
    bgColor.r -= mod(iTime, 5.45 / 8.) * vol * 4.;
...
void main() {
    ...
    vec2 vPos = -1. + 2. * (fragCoord / iResolution);
    //vPos.x += sin(iTime);
    //vPos.y += cos(iTime/2.);
    ...
    vol = texture(iChannel0, vec2(.2, .25)).r;
    ...
    if (f < maxd) {      
        ...
        vec4 tex1 = vec4(0.0); // texture2D(iChannel1, p.xy);
        vec4 tex2 = vec4(0.0); // texture2D(iChannel1, vec2(-reflect(p, scp))) / 2.;
     ...
     /*
    if (iTime > 19.0 && iTime < 31.45) {
        Color -= 1. - (21.45 - iTime) / 2.;
    }

    */

    if(vol>0.3) {
        Color -= 1. - (1. - sin(iTime)) / 2.;
    }
