#version 330 core

uniform float iTime;
uniform vec2 iResolution;

out vec4 fragColor;

// Menger & Chill
// Created by diatribes
// Shadertoy ID: 3XjSzw
// https://www.shadertoy.com/view/3XjSzw

/*
    Just a chill Menger tunnel. No stereo, but I can add
    that if anybody wants it.
    
    you're not crazy, it does move a little bit.
*/


#define T (iTime*.6)

#define W 2.
#define H 1.
#define L 180.

#define P(z) (vec3(tanh(cos((z) * .13) * .45) * 8., \
                   tanh(cos((z) * .15) * .4) * 16., (z)))
#define rot(a) mat2(cos(a), -sin(a), sin(a), cos(a))
#define N normalize
#define inf 9e9
#define TONEMAP(x) ((x) / ((x) + 0.155) * 1.019)

// Modified from Shane's Menger function
#define MENGERLAYER(scale, minmax, hole)\
    s /= (scale), \
    p = abs(fract(q/s)*s - s*.5), \
 	d = minmax(d, min(max(p.x, p.y), \
                  min(max(p.y, p.z), \
                  max(p.x, p.z))) - s/(hole))
float fractal(in vec3 q){
    vec3 p;
    float d=inf, s = 4.;

    MENGERLAYER(1., min, 3.);
    MENGERLAYER(3., max, 3.5);
    MENGERLAYER(4., max, 3.5);
    return d;
}

float tunnel(vec3 p) {
    vec2 t = P(p.z).xy;
    float s;

    s  = min(length(p.xy - t.xy),
         min(length(p.xy - t.y) ,
             length(p.xy - t.x )));

    return 1.-s;
}

float map(vec3 p) {
    return max(tunnel(p), fractal(p));
}

float AO(in vec3 pos, in vec3 nor) {
	float sca = 2.0, occ = 0.0;
    for( int i=0; i<5; i++ ){
    
        float hr = 0.01 + float(i)*0.5/4.0;        
        float dd = map(nor * hr + pos);
        occ += (hr - dd)*sca;
        sca *= 0.7;
    }
    return clamp( 1.0 - occ, 0.0, 1.0 );    
}

void main() {
    vec2 u = gl_FragCoord.xy;
    vec4 o;
    float s=.002,d=0.,i=0.,a;
    vec3  r = vec3(iResolution.xy, iResolution.y);

    u = (u-r.xy/2.)/r.y;
     
    vec3  p = P(T),ro=p,
          e = vec3(.007,0,0),
          Z = normalize( P(T+1.) - p),
          X = normalize(vec3(Z.z,0,-Z)),
          D = vec3(rot(tanh(sin(p.z*.1)*1.5)*2.)*u, 1) 
             * mat3(-X, cross(X, Z), Z);

    o -= o;

    while(i++ < L && s > .001)
        p = ro + D * d,
        p += cos(.3*T+p.yzx)*.2,
        d += (s = map(p)*.65);

    float f = mod(p.z, 30.);
    if (f > 20.)
        o.rgb = vec3(.25, .125, .35);
    else if (f > 10.)
        o.rgb = vec3(.5, .3, .1);
    else
        o.rgb = vec3(.1, .3, .5);
 
    vec3 lights = abs(o.rgb /
                  dot(cos(.25*iTime+p*.2),vec3(.01)));

    r =  N(map(p) - vec3(map(p-e.xyy), 
                         map(p-e.yxy), 
                         map(p-e.yyx)));

    o *= max(dot(r, normalize(ro-p)), .05);
    o *= AO(p, r);
    o.rgb = mix(o.rgb, lights, .001);
    o.rgb = pow(TONEMAP(o.rgb*vec3(1.5,1.,.7)*exp(-d/2.))
                - dot(u,u)*.1, vec3(.45));
    
    fragColor = o;
}

