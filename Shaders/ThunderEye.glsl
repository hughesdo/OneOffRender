...
void main(){
    ...
    vec2 uv = -1. + fragCoord / iResolution*3.;
    ...
    uv = -1. + fragCoord / iResolution*3.;// .5+2.*(fragCoord / iResolution-.5*iResolution.xy) / max(iResolution.x, iResolution.y); //reset uv, long edge -.5 to 1.5
    col = (step(.7,fft.x)<.5) ? col : mix(.66*clamp(col,.0,1.),col,1.-isBack); //set visibility

    // Output
	col = pow(col, vec3(.4545)); //gamma correction
    col *=palette(fft.x*2.+sin(iTime/10.)*2.);
