#version 330 core
// Buffer B - Render the landscape and sky...
// by David Hoskins.
...
uniform float iTime;
uniform vec4 iMouse;
uniform sampler2D iChannel0;   // Buffer A (data storage)
uniform sampler2D iChannel1;   // Texture (heightmap)
uniform sampler2D iChannel2;   // Texture (rock)
uniform sampler2D iChannel3;   // Texture (noise)
...
vec4 getStore(int num)
{
	...
	return  texelFetch(iChannel0, loc, 0);  // Buffer A is on iChannel0
}
...
float noise( in vec3 p )
{
	...
	vec2 rg = textureLod( iChannel3, (uv+ 0.5)/256.0, 0.0).yx;  // Noise texture
...
float map( in vec3 p, float di)
{
    ...
    // Grab texture based on 3D coordinate mixing...
 	float te = textureLod(iChannel1, p.xz*.0022 + p.xy * 0.0023-p.zy*.0011, di).x*40.0;  // Heightmap texture
...
{
    specular  = .8;
    vec3 alb  = texCube(iChannel2, pos*.03, nor);  // Rock texture
...
//--------------------------------------------------------------------------
// Shadowing for terrain. This version ignores the crow entirely so that no
// bird-related SDFs can influence the landscape lighting.
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )  
{
	...
	// Setup camera ray for the ray 'dir' (simple perspective, no fisheye)....
	mat3 camMat = setCamMat(camPos, camTar, (camTar.x - camPos.x) * .02);
	vec3 dir    = camMat * normalize(vec3(uv, 1.0));

	// The sky is a general mix of blue to fog colour with 3D 'cold' clouds,
	// for mixing with the distance fogging effect...
    ...
    if (dhit < FAR)
    {
        ...
        float pixel = iResolution.y;
       	...
       	vec3 mat = albedo(p, nor);
			...
			// Distance fog: smoothly blend terrain with sky using a simple
			// smoothstep over [0, FAR] to avoid hard distance bands.
			float fogFactor  = clamp(dhit / FAR, 0.0, 1.0);
			float surfaceVis = smoothstep(0.0, 1.0, 1.0 - fogFactor);
			temp             = mix(sky, temp, surfaceVis);
	      	col              = temp;
    }else
	{
