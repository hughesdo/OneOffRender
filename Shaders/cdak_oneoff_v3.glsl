...
//
// V3: camera recenter — x/y wobble in w() is smoothly damped to near-zero
// by t=80s so the camera stays looking forward for the full render.
...
float3x3 w() {
        float3 wobble = sin(float3(5,6,7)*(h.z*100.0+t*.002))*.08*(2.0-sin((h.z*2222.0-h.w)));
        // Smoothly recenter x/y tilt — damp to 8% by t=80s so camera looks forward
        float recenter = 1.0 - smoothstep(20.0, 80.0, t) * 0.92;
        wobble.xy *= recenter;
        return r(float3(0,0,smoothstep(-7.,30.,t))+smoothstep(170.,300.,t)*float3(1,2,3)+wobble);
}
