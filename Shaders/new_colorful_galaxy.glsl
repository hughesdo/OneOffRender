...
vec2  resolution = iResolution.xy;
  ...
  vec2 coord = fragCoord * 2. - iResolution.xy;
    ...
    for (int i = 0; i<45; i++) {
        ...
        vec3 col = 0.5 + 0.5*cos(iTime+uv.xyx+vec3(float(i),2.*float(i)+4.,4.*float(i)+16.));// * audio;
        ...
        final += mix(vec3(0),col, d)*holeFade(iTime,holeLife,lifeOffset)*audio;
    }
	audio_avg/=45.;
...
void main() {
	...
	vec2 fragCoord2 = (fragCoord / iResolution) * iResolution;
	mainImage(fragColor, fragCoord2);
}
