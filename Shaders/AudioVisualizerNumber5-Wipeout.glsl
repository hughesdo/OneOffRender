...
// URL: https://www.shadertoy.com/view/ddXfzf
...
void main() {
    ...
    //vec2 R = -1.0 + 2.0* (fragCoord / iResolution);
    //vec2 u = (F+F-R)/R.y * rot(-iTime*.15);
    vec2 u = (-1.0 + 2.0*(fragCoord / iResolution)) * rot(-iTime*.15);
    vec3  p, q, id;
    float i = 0., t = 0., d, f, v = getVol(8.);

    for (fragColor *= i; max(i,t) < 60.; i++) {
        ...
        fragColor += pow(f,1.5) * abs(v-id.x*.004) * .04 / (.8 + abs(d))
