...
// Audio-Reactive Colorful Columns - Enhanced Color Version with More Colors & Randomness
...
// Organized audio-reactive column heights based on distance from camera (frequency spectrum)
float getAudioHeight(vec2 cellID, vec3 worldPos, vec3 cameraPos) {
    ...
    // Sample the audio texture at the calculated frequency
    float audioLevel = 0.0;
    
    // Primary frequency sample - stronger sampling
    audioLevel = max(audioLevel, texture(iChannel0, vec2(freq, 0.0)).r);
    audioLevel = max(audioLevel, texture(iChannel0, vec2(freq, 0.25)).r);
    audioLevel = max(audioLevel, texture(iChannel0, vec2(freq, 0.5)).r);
    audioLevel = max(audioLevel, texture(iChannel0, vec2(freq, 0.75)).r);
    
    // Add some neighboring frequency data for smoother response
    float freqSpread = 0.03;
    audioLevel = max(audioLevel, texture(iChannel0, vec2(clamp(freq - freqSpread, 0.0, 1.0), 0.0)).r);
    audioLevel = max(audioLevel, texture(iChannel0, vec2(clamp(freq + freqSpread, 0.0, 1.0), 0.0)).r);
    ...
    audioLevel = max(audioLevel, testPattern * 0.2); // Weak but always present
    
    // Add bass boost for closer columns and treble emphasis for farther ones
    if (normalizedDistance < 0.3) {
        // Bass boost for close columns
        audioLevel *= 1.3;
    } else if (normalizedDistance > 0.7) {
        // Treble emphasis for far columns
        audioLevel *= 1.2;
    }
    ...
    // Amplify audio response but keep it reasonable
    audioLevel = pow(audioLevel, 0.8) * 1.5; // Less aggressive than before
    
    // Smooth and amplify the audio response
    audioLevel = smoothstep(0.0, 1.0, audioLevel);
...
void main() {
    ...
    for (int i = 0; i < RAY_STEPS; i++) {
        ...
        if (r.d < HIT_THRESHOLD) {
            ...
            // Enhanced color with audio reactivity and more saturation
            vec3 finalColor = baseColor * (1.0 + audioLevel * 0.8);
