...
void main() {
    ...
    if (y == 0) {
        ...
        for (int i = 0; i < 8; i++) {
            bass += texture(iChannel1, vec2(float(i) / 512.0, 0.25)).x;
        }
        bass /= 8.0;

        // Mids: 0.1-0.4
        for (int i = 20; i < 80; i++) {
            mid += texture(iChannel1, vec2(float(i) / 512.0, 0.25)).x;
        }
        mid /= 60.0;

        // Treble: 0.4-0.8
        for (int i = 100; i < 200; i++) {
            treble += texture(iChannel1, vec2(float(i) / 512.0, 0.25)).x;
        }
        treble /= 100.0;
