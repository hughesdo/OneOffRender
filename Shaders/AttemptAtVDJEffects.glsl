...
// URL: https://www.shadertoy.com/view/XctGzf
// License: CC0
...
const int[] g_script = int[29](
  ...
  , SCRIPT(EFFECT4, FREQ1   , FLASH)
  , SCRIPT(EFFECT0, FREQ1   , FLASH)
  , SCRIPT(EFFECT3, FREQ1, FLASH)
  , SCRIPT(EFFECT3, FREQ2, FLASH)
  , SCRIPT(EFFECT2, FREQ2   , FLASH)
  , SCRIPT(EFFECT2, FREQ2   , FLASH)
  , SCRIPT(EFFECT1, FREQ2   , FLASH)
  , SCRIPT(EFFECT1, FREQ2   , FLASH)
  , SCRIPT(EFFECT1   , FREQ1, NONE )
  , SCRIPT(EFFECT2   , FREQ2, NONE )
  );
...
vec3 thecomputer(vec3 col, vec2 p, vec2 pp) {
  ...
  scol4 += srcol4*sfre34;
  vec3 gcol = 0.0001*sbase/(1.00001+dot(srd, vec3(0.0, 0.0, 1.)))*smoothstep(aa, -aa, d.z);


  col = mix(col, gcol, smoothstep(aa, -aa, d.z));
  col = mix(col, scol4, smoothstep(aa, -aa, d.x));
  col = mix(col, scol, smoothstep(aa, -aa, d.y));
  col += glowCol/max(d.w, 1E-3);
  col += 3E-2*sqrt(seye)*(pow(voice, 3.))/max(sqrt(length(p)), 5E-1);
  col -= 6E-2*vec3(2.,3.,1.)*dot(p, p);
  return col;
}
...
vec3 freq(vec3 col, vec2 p, vec2 pp, float off) {
  ...
  float sy = sign(p.y);
  vec3 fcol = (1.5+0.5*sy)*(0.5+0.5*sin(vec3(0.,1.,2.)+abs(p.x)+TIME+0.125*sy));
...
vec3 effect(vec2 p, vec2 pp) {
  ...
  int script    = g_script[int(mod(ntime/8., 29.))];
  ...
  col = aces_approx(col);
  if (ntime >= 118. && ntime < 120.) {
      col *= 0.;
  }
  col *= smoothstep(sqrt(2.), sqrt(0.5), length(pp));
  col = sqrt(col);
  return col;
}
