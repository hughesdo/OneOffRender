...
// URL: https://www.shadertoy.com/view/4l2GW1
...
#define alarm clamp(iTime-10.0,0.0,1.0)
...
void main()
{
    ...
    vec4 color3 = texture2D(iChannel0, uv);
    ...
    vec3 p = vec3(2.+sin((iTime-10.0)) *(sin(iTime)+clamp(iTime-10.0,0.0,4.0)),
                  srand3(vec3(0.0,iTime*2.0,0.0)).xy*alarm*0.2-alarm*0.1);//3D Position

    mat3 cm = calcLookAtMatrix(p,m,0.0);//Camera matrix
    #ifdef ENDLESS
        p = vec3((iTime-10.0)*(2.0+clamp(iTime-10.0,0.0,4.0)),srand3(vec3(0.0,iTime*2.0,0.0)).xy*alarm*0.2-alarm*0.1);//3D Position
