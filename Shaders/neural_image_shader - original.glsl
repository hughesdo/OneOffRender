// Fork from https://www.shadertoy.com/view/7sSSDd
// Created by Quasimondo
// based on the work of Blackle Mori 
// Modified by ArthurTent
// - added some reactions based on the audio input
//    - zoom based on snd value
//    - devided vec4 f0_0 with snd
//    - multiplied fragColor with snd
// - removed mouse input and made cam movement based on snd, iAmplifiedTime and iTime
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    iAmplifiedTime = iTime + snd;

    vec2 uv = fragCoord/iResolution.xy;

    vec2 pn =  uv.yx+vec2(snd*cos(iAmplifiedTime/2.),sin(iTime/3.));//*1.+snd*2.;
    pn.y -= 0.2;
    fragColor = r(( (uv)*vec2(2.0,-2.0)+vec2(-1.,1.0))*vec2(iResolution.x/iResolution.y,1.0),iAmplifiedTime,pn) *.6+snd;
