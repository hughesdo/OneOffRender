...
// URL: https://www.shadertoy.com/view/XsySDW
...
vec3 render(inout vec3 o, inout vec3 r, inout float dist, in int steps) {
    ...
    if (p.y > .0) {
        ...
        color += saturate2(sin(p.x) * sin(p.z) * 50. + sin(iTime) * 10. + .5);
    }
    ...
    // add specular light
    color += saturate2(vec3(pow(clamp(dot(light, ref), 0., 1.), 15.)) / 2.);
...
void main()
{
    ...
    vec2 mouse = vec4(iResolution.xy * 0.5, 0.0, 0.0).xy / iResolution.xy * 2. - 1.;
    if (vec4(iResolution.xy * 0.5, 0.0, 0.0).x == 0. && vec4(iResolution.xy * 0.5, 0.0, 0.0).y == 0.) mouse = vec2(0);
