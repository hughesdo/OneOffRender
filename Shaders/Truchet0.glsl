...
// CC0: Truchet + Kaleidoscope FTW - Rainbow Edition
// iChannel0 = Audio (optional)
...
// === AUDIO REACTIVITY ===
// Simple loudness from FFT-style texture in iChannel0
float getLoudness() {
    // bass, mids, highs
    float bass  = texture(iChannel0, vec2(0.03, 0.0)).r;
    float mids  = texture(iChannel0, vec2(0.15, 0.0)).r;
    float highs = texture(iChannel0, vec2(0.40, 0.0)).r;

    float lvl = bass*1.6 + mids*1.0 + highs*0.7;
    // Add a small baseline so the effect is never totally dead
    return clamp(lvl*1.2 + 0.05, 0.0, 1.0);
}
...
vec4 plane(vec3 ro, vec3 rd, vec3 pp, vec3 off, float aa, float n) {
  ...
  // AUDIO: drive color and line emphasis from loudness
  float audio = getLoudness();

  // Rainbow color per wheel, with audio-driven spin and brightness
  float hue = fract(TIME * COLOR_SPEED * (0.5 + 1.7*audio) + n * COLOR_VARIATION);
  float brightness = COLOR_BRIGHTNESS * (0.4 + 1.6*audio);
  vec3 wheelColor = hsv2rgb(vec3(hue, COLOR_SATURATION, brightness));
  
  // Slight line response to level
  lw *= mix(0.8, 1.6, audio);

  // Original rendering, now using wheelColor
  vec3 col = mix(wheelColor, vec3(0.0), smoothstep(aa, -aa, d));
...
vec3 effect(vec2 p, vec2 q) {
  // AUDIO: use loudness to breathe the camera and zoom
  float audio = getLoudness();

  float tm  = TIME*0.25 + 0.20*audio*sin(TIME*2.5);
  vec3 ro   = offset(tm);
  vec3 dro  = doffset(tm);
  ...
  float zoomPulse = 1.0 + 0.30*audio;
  vec3 col = color(ww, uu, vv, ro, p*zoomPulse);
