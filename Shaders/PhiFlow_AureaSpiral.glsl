...
// Shadertoy-compatible fragment shader with rich audio reactivity
// ========== TWEAKABLE VARIABLES ==========
// Main Parameters
...
void main() {
    ...
    uv = mat2(cos(rotSpeed * t), -sin(rotSpeed * t), sin(rotSpeed * t), cos(rotSpeed * t)) * uv;
    ...
    float audioScale = 0.015 * (1.0 + 0.3 * sin(t * 0.2) + treble * TREBLE_SCALE_AMOUNT);
    ...
    vec3 bgPulse = palette(t * 0.1, paletteId) * bass * 0.02;
    col += bgPulse;
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        // Audio-reactive disc size and blur
        float baseRad = 0.015 + 0.01 * sin(fi * 0.21 + t);
        float audioRad = baseRad * (1.0 + bass * 0.5 + sin(fi * 0.1 + t) * mid * 0.3);
        float audioBlur = 0.005 * (1.0 + treble * 2.0);
        ...
        float intensity = 1.0 + treble * 0.8 + sin(fi * 0.03 + t) * bass * 0.4;
        c *= intensity;
    ...
    // Audio-reactive normalization and bloom
    col /= max(acc, 1e-3);
    col = pow(col, vec3(0.9 - overall * 0.1)); // Brighter with loud audio
