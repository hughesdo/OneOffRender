...
// Original author: ChunderFPV
// Modified by: ArthurTent for ShaderAmp project
// URL: https://www.shadertoy.com/view/McsXz8
...
void camera(vec2 fragCoord, out vec3 ro, out vec3 rd, out mat3 t)
{
    ...
    //float ff = min(1.0, step(0.001, vec4(iResolution.xy * 0.5, 0.0, 0.0).x) + step(0.001, vec4(iResolution.xy * 0.5, 0.0, 0.0).y));
    float ff = min(1.0, step(0.001, vec4(iResolution.xy * 0.5, 0.0, 0.0).x) + step(0.001, vec4(iResolution.xy * 0.5, 0.0, 0.0).y))+sin(iTime/20.);
    vec2 m = PI*ff + vec2(((vec4(iResolution.xy * 0.5, 0.0, 0.0).xy + 0.1) / iResolution.xy) * (PI*2.0));
    //m.y = -m.y;
    m.y = sin(m.y*0.5)*0.3 + 0.5;

    //vec2 sm = sin(m)*sin(iTime), cm = cos(m)*(1.+sin(iTime));
    vec2 sm = sin(m)*(1.+sin(iTime/10.)/2.);
    vec2 cm = cos(m);
...
void mainImage( out vec4 C, in vec2 U )
{
    ...
    snd2 = snd*1.3;
    //snd*=2.;
    vec2 cam_uv = -1.0 + 2.0 *(gl_FragCoord.xy / iResolution);
    ...
    vec2 R = iResolution.xy,
         u = (U+U-R)/R.y*2.,
         m = (vec4(iResolution.xy * 0.5, 0.0, 0.0).xy*2.-R)/R.y;
    ...
    /*
    if (vec4(iResolution.xy * 0.5, 0.0, 0.0).z < 1.) // not clicking
    ...
    */
    ...
    snd2*=1.1;
    {
        ...
        p = vec3(0, O(o, .618, 1.0), O(o, 1.0, -.618));
        //p.z+=(1.+FFT(1));
        p.x+=sin(iTime);
