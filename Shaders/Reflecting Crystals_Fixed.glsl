...
// Reflecting Crystals - OneOffRender Version (Audio Reactivity Removed)
// original: https://www.shadertoy.com/view/7l3SRB
// Converted for OneOffRender - static version without audio reactivity
...
float map(vec3 p) {
    ...
    // Static movement of the branches (no audio reactivity)
    vec2 dq = mix(vec2(-0.2), vec2(0.27, 0.1), sqWave(time * 0.15));
    float a = 1.;
    for(int i = 0; i < 5; i++) { // IFS.
        vec3 v = q;
        v.zx = abs(v.zx);
        if(v.z > v.x) v.zx = v.xz;
        // Static crystal thickness and size
        float thickness = 0.1;
        float crystalSize = 0.3;
        d = min(d, max(v.x - thickness, (v.x * 2. + v.y) / sqrt(5.) - crystalSize) / a);
        q.zx = abs(q.zx);
        rotpi4(q.xz);
        q.xy -= dq;
        rotpi4(q.yx);
    ...
    // Add a gyroid sphere with static size and rotation
    q = p - pos;
    float sphereSize = 0.3;
    float t = length(q) - sphereSize;
    q *= 15.;
    // Static gyroid rotation (no audio reactivity)
    q.xy *= rot(time * 1.3);
    q.yz *= rot(time * 1.7);
    float gyroidIntensity = 0.2;
    t = max(t, (abs(dot(sin(q), cos(q.yzx))) - gyroidIntensity) / 15.);
    // Static smooth minimum
    float smoothness = 0.3;
    d = smin(d, t, smoothness);
...
vec3 march(inout vec3 rp, inout vec3 rd, inout vec3 ra, inout bool hit) {
    ...
    // Static color phase (no audio reactivity)
    float phase = length(rp) * 4. - time * 2.;
    // Static color with no frequency-based hue shifts
    float hueShift = 0.0;
    vec3 al = hsv(floor(phase / pi) * pi * 0.4 + hueShift, 0.8, 1.); // Albedo.
    ...
    // Static light power (no audio reactivity)
...
void main()
{
    ...
    // Static gyroid sphere movement (no audio reactivity)
    pos = sin(vec3(13, 0, 7) * time * 0.1);

    vec3 col = vec3(0); // Color.

    // Static camera position and rotation (no audio reactivity)
    ...
    // Static color (no audio reactivity)
    col *= 1.0;
