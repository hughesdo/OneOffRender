...
void main() 
{
    ...
    vec2 uv = -1.0 + 2.0 *fragCoord / iResolution +.5;
    uv.y -= 0.5;
    ...
    //fragColor += pow(max(fragColor - .2, 0.0), vec4(1.4))*vec4(sin(iTime), cos(iTime), 1., 1.);
    fragColor += pow(max(fragColor - .4, 0.15), vec4(1.4))*vec4(vec3(0.5-(cos(iTime)+sin(iTime)), sin(iTime)*.5, cos(iTime)*5.),1.);
}