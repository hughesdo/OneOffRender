...
// Original author: sukupaper
// Modified by: ArthurTent for ShaderAmp project
// URL: https://www.shadertoy.com/view/7lBXRG
...
float pattern( vec2 p ) {
    float bla = penta_or_heart+fbm( p*snd*2. + fbm( p + fbm(p*(.5+sin(iTime))*(1.+snd*2.)) ) )*snd*10.; 
    //float bla = fbm( p*snd*2. + fbm( p + fbm(p*(.5+sin(iTime))) ) );//*snd*10.; 
    ...
    //float bla = testPattern(p)+fbm( p*snd*2. + fbm( p + fbm(p*(.5+sin(iTime))) ) )*snd*10.; 
    //float bla = penta_or_heart+fbm( p*snd*2. + fbm( p + fbm(p*(.5+sin(iTime))) ) )*snd*10.; 
    //float bla = penta_or_heart+fbm( p*snd*2. + fbm( p + fbm(p*(.5+sin(iTime))) ) );//*snd*10.; 
    float bla = penta_or_heart+fbm( p*snd*2. + fbm( p + fbm(p*(.5+sin(iTime))) ) );//*snd*10.; 
    //if(penta_or_heart==0.){
    //if(mod(iTime, 5.)>2.){
	...
	return 0.;
	*/
    //return fbm( p*snd*2. + fbm( p + fbm(p*(.5+sin(iTime))) ) )*snd*10.;
}
