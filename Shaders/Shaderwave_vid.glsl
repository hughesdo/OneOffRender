...
uniform sampler2D iChannel1;  // Font texture
uniform sampler2D iChannel2;  // Video texture
...
float getLetter(int lid, vec2 uv){
	...
	for(int i = -1; i < 2; ++i){
		for(int j = -1; j < 2; ++j){
			...
			accum += texture(iChannel1, fontUV+offset, 0.0).a;
		}
	}
	return accum/9.0;
}
...
void main()
{
	...
	if(uv.y < gridHeight){
		...
		finalColor = mat3(getCol(colId),getCol(colId+1.),getCol(colId+2.)) * finalColor*10.;
	...
	vec3 resultColorWithBorder = mix(vec3(0.),vec3(fragColor.x, fragColor.y, fragColor.z),pow(max(0.,1.5-length(uvCenter*uvCenter*uvCenter*vec2(2.0,2.0))),.3));
