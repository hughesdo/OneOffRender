...
// URL: https://www.shadertoy.com/view/ld23Wd
...
vec3 getTexture(vec2 p){
	vec4 tex = texture(iChannel1, p);
    return tex.xyz * tex.w;
}
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	{
		colorize[int(i*3.0)]+=texture(iChannel0,vec2(i,0.0)).y*pow(i+.5,.9);
	}
	...
	osc += texture(iChannel0,vec2(offset+tuv.x*.4 -.01,1.0)).x-.5;
	osc += texture(iChannel0,vec2(offset+tuv.x*.4 +.01,1.0)).x-.5;
	osc*=.333;
	float boost = texture(iChannel0,vec2(.0)).x;
	...
	float oint = blockint = 
		-    texture(iChannel0,vec2(blockint-.02,.0)).x
		+2.0*texture(iChannel0,vec2(blockint,.0)).x
		-    texture(iChannel0,vec2(blockint+.02,.0)).x;
	...
	#ifdef VIDEO
			vec4 vid = texture(iChannel1, buv);
			//fragColor+=vid;

			fragColor *= vec4(texture(iChannel1,(gl_FragCoord.xy / iResolution)).rgb, .1);
    ...
    vec3 col1 = vec3(sin(iTime)/1.5 + 10.8 * best_acc, 0.3 + 0.2 * best_acc, 0.2 + 0.3 * best_acc) * pow(best_acc, 2.0) * 3.0;
