...
#define T           iTime
#define M           vec4(0.0)  // Mouse not available in OneOffRender
...
void main() {
    ...
    //mouse rotation (disabled in OneOffRender)
    float x = -.305;
    float y = -.750;

    // Mouse not available in OneOffRender, use fixed rotation
    // if(M.z>0.){
    //     x = -(M.y/R.y * .5 - .25) * PI;
    //     y = -(M.x/R.x * .5 - .25) * PI;
    // }
