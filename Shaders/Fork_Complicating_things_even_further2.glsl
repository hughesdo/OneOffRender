...
// Original author: Pink
// Modified by: ArthurTent for ShaderAmp project
// URL: https://www.shadertoy.com/view/csfyzS
// License: CC0
...
vec4 plane(vec3 ro, vec3 rd, vec3 pp, vec3 off, float aa, float n) {
  float vol =  texture(iChannel0, vec2(0., 0.14)).x;
...
//  float rdd = 2.0;
  ...
  for (int i = 1; i <= furthest; ++i) {
    //float vol =  texture(iChannel0, vec2(i, 0.25)).x;
    float vol =  texture(iChannel0, vec2(i, 0.0)).x;
