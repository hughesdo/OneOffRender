...
// Original author: derSchamane
// Modified by: ArthurTent for ShaderAmp project
// URL: https://www.shadertoy.com/view/ctcGRf
// License: CC0
...
vec2 Map(in vec3 p) //Thanks dracusa, nice aticle <3
{
    ...
    vec2 mouse = iResolution.xy * 0.5;
    float t = iTime/7. + mouse.x/iResolution.x*3.;
    p.x -= t;
...
void main()
{
    ...
    hi = avgFrq(0.7,1.);
	vec2 m = (iResolution.xy * 0.5) / iResolution.xy;
    if (length(m) <= .1) m = vec2(.5);

    vec3 ro = vec3(0, 0, -MAX_DIST/2.);
    ro.yz = Rot2D(ro.yz, -m.y * PI + PI*.5*sin(iTime/10.));
    //ro.yz = Rot2D(ro.yz, -m.y * PI + PI*.5*iTime/10.*fft.x/4000.);
    ro.xz = Rot2D(ro.xz, -m.x * PI*2. - PI*.5*sin(iTime/10.));
    ...
    //float colId = 2. * floor(mod(iTime/4.+fft.x*2.,4.));
    float colId = 2. * floor(mod(iTime/8.,4.));
