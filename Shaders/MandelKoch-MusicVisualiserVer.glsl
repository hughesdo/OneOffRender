...
// URL: https://www.shadertoy.com/view/sslXzX
...
// Simplified date replacement for OneOffRender (no iDate available)
float d = 1.0;
return d;
...
{
    float fr_avg = (freqs[0]+freqs[1]+freqs[2]+freqs[3])/4.;
    #if 1
    {
        float c2 = dot(c, c);
...
vec3 mandelbrotImg(vec2 p)
{
    ...
    float mtime =  time;
    mtime -= freqs[3];
    float zoo = 0.62 + 0.38*cos(.1*mtime)*fr_avg;
   ...
   vec2 c = vec2(-.745,.186) + xy*zoo;
    ...
    vec3 col1 = vec3(0);
    if(tsin>0.85){
        col1 = 0.5 + 0.5*cos(3.0 + l*.15 * randomCol(.1)*fr_avg*4.);
    }else if (tsin>0.5){
        col1 = 0.5 + 0.5*cos(3.0 + l*.15 * randomCol(.1)+fr_avg*4.);
    }else{
        col1 = 0.5 + 0.5*cos(3.0 + l*.15 * randomCol(.1));
    }
    //col1 = 0.5 + 0.5*cos(3.0 + l*.15 * randomCol(.1))*fr_avg*4.;
    #ifdef EXTRA_DMT
...
//--------image part---------
	...
	vec2 mouse = 1. - (iResolution.xy * 0.5)/iResolution.xy; // 0 1
	vec2 ouv = uv;
    //uv.y -= .05;
