...
// URL: https://www.shadertoy.com/view/XtKXDh
...
void camera(vec2 fragCoord, out vec3 ro, out vec3 rd, out mat3 t)
{
    ...
    float ff = min(1.0, step(0.001, mouse.x) + step(0.001, mouse.y))+sin(iTime/20.);
    vec2 m = PI*ff + vec2(((mouse.xy + 0.1) / iResolution.xy) * (PI*2.0));
    //m.y = -m.y;
...
void main() {
	...
	//camera + rd for stars
    vec3 ro2 = vec3(0.0);//rd = vec3( 0.0 );
	vec3 rd2 = normalize(vec3(uv,-1.5));
    mat3 t = mat3(1.0);
	camera(uv, ro2, rd2, t);
    ...
    uv = -1.0 + 2.0 *(fragCoord / iResolution)+.5;
    uv = uv * 2.0 - 1.0;
	...
	rd2.x+=sin(iTime/1000.)*2.;
    vec3 bg = stars(rd2)*(1.+30.*snd);

	vec3 scene = marchScene(ro, rd);
	scene +=bg;
