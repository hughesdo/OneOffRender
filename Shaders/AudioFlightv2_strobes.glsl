...
// Original author: byt3_m3chanic
// Modified by: ArthurTent for ShaderAmp project
// URL: https://www.shadertoy.com/view/7tfyRl
...
#define T           iTime
#define M           vec4(iResolution.xy * 0.5, 0.0, 0.0)
...
vec2 map (in vec3 p, float sg) {
    ...
    float obj = fBox(o,vec3(.15*offWobble))-0.2015*sampleFreq(1.5);
    if(obj<res.x ) {
        res = vec2(obj,11.);
        g_hp=o;
    }
...
void main() {
    ...
    //vec2 uv = (2.0 * (fragCoord / iResolution) - 1.0) * vec2(iResolution.x / iResolution.y, 1.0);
