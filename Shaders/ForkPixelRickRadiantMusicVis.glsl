...
// Original author: ArthurTent
// Modified by: ArthurTent for ShaderAmp project
// URL: https://www.shadertoy.com/view/MXdGz4
...
vec3 eyes(vec3 color, vec2 p)
{
    ...
    // pupil
    d = 0.2 + 0.1 * cos(atan(p.y, p.x)* 10.0)-texture(iChannel0,vec2(abs(fract(5.*size/6.283)*2.-1.),.75)).r/2.;
...
float mouthLine(vec2 p, float radius, float thickness)
{
    ...
    float rb = thickness; //0.15*(0.5+0.5*cos(iTime*0.41+3.0));
    //float rb = 0.15*(0.025+0.025*cos(iTime*0.41+3.0));
...
void main()
{
    ...
    vec2 cam_uv = -1.0 + 2.0 *(fragCoord / iResolution);
	//camera + rd for stars
